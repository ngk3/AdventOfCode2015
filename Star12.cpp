...
// Function that counts total brightness lights given the maximum x and y (max_x, max_y)
int getTotalBrightness(unordered_map<string, int> lights, int max_x, int max_y) {
	int total = 0;
	for (int x = 0; x <= max_x; x++) {
		for (int y = 0; y <= max_y; y++) {
			total += lights[to_string(x) + "," + to_string(y)];
		}
	}
	return total;
}
...
void perform(vector<array<string, 3>> instructions, unordered_map<string, int>& lights) {
	...
	for (int i = 0; i < instructions.size(); i++) {
		...
		if (instructions[i][0] == "on") {
			...
			for (int x = stoi(s1_split[0]); x <= stoi(s2_split[0]); x++) {
				for (int y = stoi(s1_split[1]); y <= stoi(s2_split[1]); y++) {
					lights[to_string(x) + "," + to_string(y)] += 2;
				}
			}
		}
	}
...
int main() {
	...
	cout << "Total brightness = " << getTotalBrightness(lights, 999, 999) << endl;
}